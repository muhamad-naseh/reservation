...
@section('content')
    ...
    <form action="{{ route('admin.category.update', $category->id ) }}" method="post" enctype="multipart/form-data" class="relative w-full p-4">
        @csrf
        @method('PUT')
        <div class="mb-8">
            <label for="name" class="block mb-2 text-sm font-medium text-slate-700 after:content-['*'] after:text-pink-600 after:ml-0.5">Category Name</label>
            <input type="text" name="name" id="name" value="{{ $category->name }}" placeholder=""
        ...
        <div class="mb-5">
            <button type="submit" class="flex items-center text-gray-900 bg-white hover:bg-gray-100 border border-gray-200 focus:ring-4 focus:outline-none focus:ring-gray-100 font-medium rounded-lg text-sm px-5 py-2.5 drop-shadow-md">
                ...
                <span class="ml-2 whitespace-nowrap font-semibold">Update Data</span>
            </button>
        </div>
    </form>
@endsection
